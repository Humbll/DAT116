...
% 
%     SIGSPECTRUM(X, 'noplot') suppresses the plot completely
%     (useful when extracting the power vector; see below). 
% 
...
%    end 
    ...
    for k = 1:size(varargin')
        ...
        maxix = floor(length(spec)/2) + 1; 
        
        % add two components of same real frequency
        spoc = spec(end:-1:1, :); 
        spec(2:(maxix-1), :) = spec(2:(maxix-1), :) + spoc(1:(maxix-2), :) ; 
